...
function UnderbarForm_Calendar(props) {
  ...
  const onHealthHandler = (event) => {
    dispatch(outputPill()).then((response) => {
      console.log("백에서 get으로 받은거 출력", response.payload +"/" + props.match.params.id);
...
export default withRouter(UnderbarForm_Calendar);
