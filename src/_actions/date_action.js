...
export function inputDate(dataTosubmit) { 
  const request = axios
    .post("http://3.34.183.92:4000/api/main/date", dataTosubmit)
...
export function outputDate() {
  const request = axios
  .get("http://3.34.183.92:4000/api/main", {params:{Date_send})
  .then((response) => response.data)
  .catch((error)=>console.log(error.response.data));
...
export function todayDate() {
  const request = axios
  .get("http://3.34.183.92:4000/api/main/today", {params:{Today_send})
  .then((response)=> response.data)
  .catch((error)=>console.log(error.response.data));
